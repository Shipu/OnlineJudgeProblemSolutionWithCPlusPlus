...
//        puts("OK");
        ...
        v.col=(u.col+1)%5;
//        puts("OK");
        if(v.x>=0 && v.x<r && v.y>=0 && v.y<c && s[v.x][v.y]!='#' && v.time < vis[v.x][v.y][v.dir][v.col])
        {
            vis[v.x][v.y][v.dir][v.col] = v.time;
            q.push(v);
        }
//        puts("OK");
        for0(i,4)
        {
            v=u;
            v.dir=(u.dir+1+i)%4;
            if(abs(u.dir-v.dir)==2)
                v.time=u.time+2;
            else
                v.time=u.time+1;

            if( v.time < vis[v.x][v.y][v.dir][v.col] )
            {
                vis[v.x][v.y][v.dir][v.col] = v.time;
                q.push(v);
            }
