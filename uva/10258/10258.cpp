...
//            PPN(i,p),P(t),CP(c);
            ...
            else if(c=='C' && check[i][p]!=-1)
            {
                judge[i].penalty+=(t+check[i][p]);
                judge[i].solved+=1;
                judge[i].id=i;
                check[i][p]=-1;
            }
        ...
        sort(judge,judge+105,cmp);

        for0(i,105)
        {
            if(judge[i].id==110)
                break;
            PPN(judge[i].id,judge[i].solved),P(judge[i].penalty);
        }
        if(t)
        nl;
    }
